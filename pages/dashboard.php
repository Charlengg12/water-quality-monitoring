...
<head>
  ...
  <style>
    ...
    .data-source-indicator .text-success { color: #28a745 !important; }
    .data-source-indicator .text-info { color: #17a2b8 !important; }
  </style>
</head>
...
<body>
  ...
  <div class="container mt-4">
    ...
    <!-- ✅ 3. Add a visual indicator for data source -->
    <div class="data-source-indicator">
      <p>Data Source: <span id="dataSource">Connecting...</span></p>
    </div>
  ...
  <script>
    ...
    async function getLiveESP32Readings() {
        try {
            const response = await fetch(ESP32_URL, { timeout: 2000 });
    ...
    async function loadHybridReadings() {
        let data = await getLiveESP32Readings();

        if (data === null) {
            console.log("ESP32 offline → loading latest DB data.");
            data = await getDatabaseReadings();
        } else {
            console.log("Live ESP32 data loaded.");
        }
    ...
    function updateUI(data) {
        ...
        // Update gauge values
        const paramMappings = {
            'tds': ['TDS_Value', 'TDS_Status'],
            'ph': ['PH_Value', 'PH_Status'],
            'turbidity': ['Turbidity_Value', 'Turbidity_Status'],
            'lead': ['Lead_Value', 'Lead_Status'],
            'color': ['Color_Value', 'Color_Status', 'Color_Result']
        };

        for (const param in paramMappings) {
            const [valueKey, statusKey, resultKey] = paramMappings[param];
            
            let displayValue = '--';
            let displayStatus = 'N/A';
            
            if (param === 'color') {
                displayValue = (data[resultKey] !== undefined) ? data[resultKey] : 
                              (data[valueKey] !== undefined ? data[valueKey].toFixed(2) : '--');
                displayStatus = data[statusKey] || 'N/A';
            } else {
                const rawValue = data[valueKey];
                const parsedValue = parseFloat(rawValue);
                displayValue = isNaN(parsedValue) ? '--' : parsedValue.toFixed(
                    param === 'lead' ? 4 : 
                    param === 'ph' || param === 'turbidity' ? 2 : 
                    0
                );
                displayStatus = data[statusKey] || 'N/A';
                
                // Rotate needle for non-color parameters
                rotateNeedle(param, parsedValue);
            }
        ...
        if (dataSourceElement) {
            dataSourceElement.innerHTML = data.source === "esp32"
                ? "<span class='text-success'>LIVE (ESP32)</span>"
        ...
        if (statusMessageElement) {
            statusMessageElement.textContent = data.source === "esp32"
                ? `Connected to ESP32 at ${ESP32_IP}`
                : "ESP32 offline → Showing latest database readings";
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // If station selected, begin hybrid polling
      if (stationId) {
        loadHybridReadings();
        setInterval(loadHybridReadings, 5000); // 5s poll
      } else {
        ...
        startBtn.addEventListener('click', () => {
          ...
          .then(res => {
            if (res.status === 200) {
              ...
              // Refresh readings after starting test
              setTimeout(loadHybridReadings, 1000);
