...
// Use your PC's IPv4 address here so the ESP32 can reach the XAMPP backend.
// From the ESP32's point of view, "localhost" would point to the ESP32 itself, not your PC.
const char* serverUrl = "http://192.168.1.3/watermonitoringsystem-main/api_upload.php";
...
// --- COLOR SENSOR CALIBRATION CONSTANTS (REMOVED) ---
...
// ------------------------------------------------------------------
// CORE FUNCTIONS - UPDATED TO MATCH YOUR PARAMETERS EXACTLY
// ------------------------------------------------------------------

void calculateTDSStatus() {
    if (tdsValue < 9.0) {
        strcpy(tdsStatus, "Safe");
    } else if (tdsValue >= 9.0 && tdsValue < 10.0) {
        strcpy(tdsStatus, "Neutral");
    } else if (tdsValue == 10.0) {
        strcpy(tdsStatus, "Warning");
    } else if (tdsValue > 10.0) {
        strcpy(tdsStatus, "Failed");
    } else {
        strcpy(tdsStatus, "Unknown");
    }
}

void calculatePHStatus() {
    if (phValue >= 5.0 && phValue < 6.0) {
        strcpy(phStatus, "Safe");
    } else if (phValue >= 6.0 && phValue < 7.0) {
        strcpy(phStatus, "Neutral");
    } else if (phValue == 7.0) {
        strcpy(phStatus, "Warning");
    } else if (phValue < 5.0 || phValue > 7.0) {
        strcpy(phStatus, "Failed");
    } else {
        strcpy(phStatus, "Unknown");
    }
}

void calculateTurbidityStatus() {
    if (turbidityValue < 4.0) {
        strcpy(turbidityStatus, "Safe");
    } else if (turbidityValue >= 4.0 && turbidityValue < 5.0) {
        strcpy(turbidityStatus, "Neutral");
    } else if (turbidityValue == 5.0) {
        strcpy(turbidityStatus, "Warning");
    } else if (turbidityValue > 5.0) {
        strcpy(turbidityStatus, "Failed");
    } else {
        strcpy(turbidityStatus, "Unknown");
    }
}

void calculateLeadStatus() {
    if (leadValue < 0.009) {
        strcpy(leadStatus, "Safe");
    } else if (leadValue >= 0.009 && leadValue < 0.01) {
        strcpy(leadStatus, "Neutral");
    } else if (leadValue == 0.01) {
        strcpy(leadStatus, "Warning");
    } else if (leadValue > 0.01) {
        strcpy(leadStatus, "Failed");
    } else {
        strcpy(leadStatus, "Unknown");
    }
}

void calculateColorStatus() {
    if (tcuValue < 9.0) {
        strcpy(colorStatus, "Safe");
    } else if (tcuValue >= 9.0 && tcuValue < 10.0) {
        strcpy(colorStatus, "Neutral");
    } else if (tcuValue == 10.0) {
        strcpy(colorStatus, "Warning");
    } else if (tcuValue > 10.0) {
        strcpy(colorStatus, "Failed");
    } else {
        strcpy(colorStatus, "Unknown");
    }
}
...
void analyzeColor() {
    ...
    // Color units from raw sensor data (simplified)
    tcuValue = c_raw / 100.0; // Simple scaling
...
// Updated readAllSensors function - use direct analog readings
void readAllSensors() {
    // Read TDS sensor (direct analog reading)
    tdsValue = analogRead(TDS_PIN) / 100.0; // Simple scaling
    
    // Read pH sensor (direct analog reading)
    phValue = analogRead(PH_PIN) / 100.0; // Simple scaling
    
    // Read Turbidity sensor (direct analog reading)
    turbidityValue = analogRead(TURBIDITY_PIN) / 100.0; // Simple scaling
    
    // Read Lead sensor (direct analog reading)
    leadValue = analogRead(LEAD_PIN) / 10000.0; // Simple scaling for lead
